...
function WriterLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch('https://openscroll-backend.onrender.com/api/auth/login', {
        method: 'POST',
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setUserContext({ name: user.displayName, email: user.email, role: 'writer' }, token);
      navigate('/writer/dashboard');
    } catch (error) {
      ...
      <div style={leftSide(isMobile)}>
        <div>
          ...
          <div style={smallText}>
            ...
            <Link to="/writer/signup" style={{ color: '#d0f330', fontWeight: '600' }}>
              Sign Up
            </Link>
          </div>
